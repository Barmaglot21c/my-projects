...
vector<JSAMPLE> SaveImageToScanline(const Image& in_image, int y) {
  const Color* line = in_image.GetLine(y);
    vector<JSAMPLE> buffer(in_image.GetWidth() * 3);
    
    for (int x = 0; x < in_image.GetWidth(); ++x) {
        buffer[3 * x] = static_cast<JSAMPLE>(line[x].r);
        buffer[3 * x + 1] = static_cast<JSAMPLE>(line[x].g);
        buffer[3 * x + 2] = static_cast<JSAMPLE>(line[x].b);
    }
...
bool SaveJPEG(const Path& file, const Image& image) {
  ...
  while (cinfo.next_scanline < cinfo.image_height) {
    ...
    vector<JSAMPLE> image_buffer = SaveImageToScanline(image, cinfo.next_scanline);
