...
class Expr {
 public:
  ...
  virtual double Evaluate(const FormulaAST::ValueGetter& fn_get_cell_value) const = 0;
...
class BinaryOpExpr final : public Expr {
 ...
 public:
  ...
  double Evaluate(const FormulaAST::ValueGetter& fn_get_cell_value) const override {
    double res;
    double lhs = lhs_->Evaluate(fn_get_cell_value);
    double rhs = rhs_->Evaluate(fn_get_cell_value);
    switch (type_) {
      case Type::Add:
        res = lhs + rhs;
        break;
    ...
    return std::isfinite(res) ? res : throw FormulaError(FormulaError::Category::Arithmetic);
  }
...
class UnaryOpExpr final : public Expr {
 ...
 public:
  ...
  double Evaluate(const FormulaAST::ValueGetter& fn_get_cell_value) const override {
    return type_ == Type::UnaryPlus ? operand_->Evaluate(fn_get_cell_value)
                                    : -operand_->Evaluate(fn_get_cell_value);
  }
...
class CellExpr final : public Expr {
 public:
  ...
  double Evaluate(const FormulaAST::ValueGetter& fn_get_cell_value) const override {
    return fn_get_cell_value(*cell_);
  }
...
double FormulaAST::Execute(const ValueGetter& fn_get_cell_value) const {
  return root_expr_->Evaluate(fn_get_cell_value);
}
